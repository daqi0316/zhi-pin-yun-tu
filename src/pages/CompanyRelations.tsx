...
export default function CompanyRelations() {
  ...
  // 选中候选人的关联网络
  const { data: relationData, isLoading: relationLoading } =
    trpc.relation.candidateRelations.useQuery(selectedCandidateId!, {
  ...
  const relationColor = (level: string) => {
    switch (level) {
      case "strong":
        return { bg: "#06D6A012", text: "#06D6A0", label: "强关联" };
      case "medium":
        return { bg: "#F59E0B12", text: "#F59E0B", label: "中关联" };
      default:
        return { bg: "#94A3B812", text: "#94A3B8", label: "弱关联" };
    }
  };
                    ...
                    <div className="ml-auto flex gap-3">
                      <div className="text-center">
                        <div className="text-lg font-bold text-[#06D6A0]">
                          {
                            relationData.relations.filter(
                              r => r.relationLevel === "strong"
                            ).length
                          }
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {g.candidates.map((c: any) => (
                            <button
                              key={c.candidateId}
                              onClick={() =>
                                handleSelectCandidate(c.candidateId)
                              }
