...
const barsDimensions: BarsDimension[] = [
  { key: 'scoreSkill', label: '专业技能', weight: 30, descriptions: [
    '几乎不具备岗位要求的核心技能',
    '技能基础薄弱，仅停留在理论层面',
  ...
  { key: 'scoreProblem', label: '问题解决', weight: 20, descriptions: [
    '面对问题无从下手，缺乏逻辑思维',
    '只能按照既定流程解决问题，缺乏独立判断',
  ...
  { key: 'scoreCommunication', label: '沟通表达', weight: 20, descriptions: [
    '沟通困难，难以理解其意图',
    '表达混乱，抓不住重点',
    '表达基本清楚，偶尔需要追问才能理解要点',
    '表达流畅，逻辑清晰，能有效传递信息',
  ...
  { key: 'scoreTeamwork', label: '团队协作', weight: 15, descriptions: [
    '无法融入团队，有协作冲突历史',
    '合作意识薄弱，倾向于单打独斗',
    '能完成自己分内工作，但较少主动协作',
    '良好合作者，能配合团队完成任务',
  ...
  { key: 'scoreCulture', label: '文化匹配', weight: 15, descriptions: [
...
function BarsScoreInput({ dimension, value, onChange }: { dimension: BarsDimension; value: number; onChange: (v: number) => void }) {
  return (
    <div className="mb-5">
      <div className="flex items-center justify-between mb-2">
        <span className="text-sm font-semibold text-[#1E293B]">{dimension.label}</span>
        <span className="text-xs text-[#94A3B8] font-medium">权重 {dimension.weight}%</span>
      </div>
      ...
      {value > 0 && (
        <div className="px-3 py-2 bg-[#2D8FF0]/5 border border-[#2D8FF0]/10 rounded-lg">
          <div className="flex items-start gap-2">
            <div className="w-5 h-5 rounded-full bg-[#2D8FF0] text-white flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5">{value}</div>
            <p className="text-xs text-[#475569] leading-relaxed">{dimension.descriptions[value - 1]}</p>
          </div>
        </div>
      )}
    </div>
...
function BarsResultSummary({ scores }: { scores: Record<string, number> }) {
  const weights: Record<string, number> = { scoreSkill: 0.30, scoreProblem: 0.20, scoreCommunication: 0.20, scoreTeamwork: 0.15, scoreCulture: 0.15 }
  const total = Object.entries(weights).reduce((sum, [key, w]) => sum + (scores[key] || 0) * w, 0)
  const percentage = Math.round(total * 100 / 5)
  const grade = percentage >= 90 ? 'S' : percentage >= 75 ? 'A' : percentage >= 60 ? 'B' : percentage >= 40 ? 'C' : 'D'
  const gradeColor: Record<string, string> = { S: '#06D6A0', A: '#2D8FF0', B: '#F59E0B', C: '#FF5A65', D: '#94A3B8' }
    ...
    <div className="mt-4 p-4 bg-gradient-to-br from-slate-50 to-white border border-slate-200/60 rounded-xl">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          <span className="text-2xl font-bold" style={{ color: gradeColor[grade] }}>{percentage}</span>
          <span className="text-xs font-medium px-2 py-0.5 rounded-full" style={{ background: gradeColor[grade] + '20', color: gradeColor[grade] }}>{grade}级</span>
        </div>
      </div>
        ...
        {barsDimensions.map((dim) => {
          ...
          return (
            <div key={dim.key} className="flex items-center gap-2">
              <span className="text-xs text-[#94A3B8] w-16 text-right flex-shrink-0">{dim.label}</span>
              <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                <div className="h-full rounded-full transition-all duration-300" style={{ width: `${pct}%`, background: `linear-gradient(90deg, #2D8FF0, #06D6A0)` }} />
              </div>
              <span className="text-xs font-medium text-[#475569] w-6">{score}/5</span>
            </div>
          )
        })}
      </div>
      <p className="text-[10px] text-[#94A3B8] mt-2">面试得分 = 专业技能×30% + 问题解决×20% + 沟通表达×20% + 团队协作×15% + 文化匹配×15%</p>
    </div>
  )
}
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[#94A3B8] mt-1">BARS行为锚定评分法 · 结构化面试评估</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 text-sm text-[#475569]">
            <Calendar className="w-4 h-4 text-[#94A3B8]" />
            <span>待面: {interviewList.filter((i: any) => i.status === 'pending').length}</span>
          </div>
          <div className="flex items-center gap-2 text-sm text-[#475569]">
            <CheckCircle2 className="w-4 h-4 text-[#06D6A0]" />
            <span>已评: {interviewList.filter((i: any) => i.status === 'completed').length}</span>
          </div>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-100">
                ...
                <th className="text-left text-xs font-medium text-[#94A3B8] px-4 py-3">BARS评分</th>
                ...
                return (
                  <tr key={iv.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-4 py-4">
                      {hasBars ? (
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-sm font-semibold text-[#F59E0B]">{iv.totalScore}</span>
                          {iv.scoreSkill && (
                            <div className="flex gap-0.5 ml-1">
                              {[
                                ...
                                <span key={d.label} className="text-[9px] px-1 py-0.5 bg-[#2D8FF0]/10 text-[#2D8FF0] rounded font-medium">{d.label}{d.val}</span>
                              ))}
                            </div>
                          )}
                        </div>
                    ...
                    <td className="px-4 py-4">
                      {iv.status === 'pending' && (
                        <button
                          onClick={() => { setSelectedInterview(iv); setBarsScores({}); setFeedbackText(''); setShowBars(true) }}
                      ...
                      {hasBars && (
                        <button
                          onClick={() => { setSelectedInterview(iv); setShowBars(false) }}
      ...
      {selectedInterview && showBars && selectedInterview.status === 'pending' && (
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-[640px] max-h-[85vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-5">
                <div>
                  <h2 className="text-lg font-semibold text-[#1E293B]">BARS 行为锚定评分</h2>
                  <p className="text-xs text-[#94A3B8] mt-0.5">根据行为描述选择对应等级 (1-5分)</p>
                </div>
                <div className="flex items-center gap-3 bg-slate-50 rounded-xl px-4 py-2">
                  ...
                  <div>
                    <div className="text-sm font-medium text-[#1E293B]">候选人#{selectedInterview.candidateId}</div>
              ...
              <div className="flex gap-3 mt-4">
                <button
                  ...
                  disabled={!barsDimensions.every(d => barsScores[d.key] > 0) || updateScoreMutation.isPending}
                ...
                >
                  {updateScoreMutation.isPending ? '提交中...' : '提交 BARS 评分'}
                </button>
                <button
                  onClick={() => { setSelectedInterview(null); setBarsScores({}); setFeedbackText('') }}
      ...
      {selectedInterview && !showBars && selectedInterview.status === 'completed' && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center">
          <div className="absolute inset-0 bg-black/30" onClick={() => setSelectedInterview(null)} />
          <div className="relative bg-white rounded-2xl shadow-2xl w-[640px] max-h-[85vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-5">
                <h2 className="text-lg font-semibold text-[#1E293B]">面试评分详情</h2>
                <button onClick={() => setSelectedInterview(null)} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-slate-100 text-slate-400">✕</button>
              </div>
              <div className="flex items-center gap-3 bg-slate-50 rounded-xl px-4 py-3 mb-4">
                ...
                <div className="ml-auto flex items-center gap-1.5">
                  ...
                  <span className="text-xl font-bold text-[#F59E0B]">{selectedInterview.totalScore}</span>
                </div>
              </div>
                  ...
                  return (
                    <div key={dim.key} className="p-3 bg-slate-50 rounded-xl">
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="text-sm font-medium text-[#1E293B]">{dim.label} <span className="text-[#94A3B8] font-normal">({dim.weight}%)</span></span>
                        <span className="text-sm font-bold text-[#2D8FF0]">{score}/5</span>
                      </div>
                      <div className="h-1.5 bg-slate-200 rounded-full overflow-hidden mb-2">
                        <div className="h-full rounded-full bg-gradient-to-r from-[#2D8FF0] to-[#06D6A0]" style={{ width: `${score * 20}%` }} />
                      </div>
                      <p className="text-xs text-[#475569] leading-relaxed">{dim.descriptions[score - 1]}</p>
                    </div>
                  )
                })}
              </div>
              {selectedInterview.feedback && (
                <div className="mt-4 p-3 bg-slate-50 rounded-xl">
                  <span className="text-sm font-medium text-[#1E293B]">评价备注</span>
                  <p className="mt-1 text-sm text-[#475569]">{selectedInterview.feedback}</p>
                </div>
              )}
